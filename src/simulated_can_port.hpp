...
{
public:
    SimulatedCanPort(uint8_t node_id, ThreadXCia402Logger *logger = nullptr);
    ~SimulatedCanPort() override = default;
...
private:
    ...
    bool SendBootup();
    bool SendHeartbeat(uint8_t state);
    bool Notify(const can_msg &msg) const;
